...
3. **Skills** — category-grouped mono chip visual with hover-lime, scroll-reveal. Visualized, not a flat list. **[API]**
4. **Experience** — scroll-linked animated timeline: vertical line draws itself as the user scrolls, single lime pulse travels down, each role dot lights up as the pulse passes. Calm, one pulse, respects prefers-reduced-motion. **[API]**
5. **Projects / Case Studies** — unified: card grid is the index, each card's detail page IS the full case study. "Case Studies" nav item scrolls to the featured subset (Sales CRM, PureKeep). No separate duplicate section. **[API]**
6. **The AI Chatbot** — "Ask my portfolio anything" — persistent launcher + RAG pipeline. (see §6)
7. **Contact** — form **[API write]**, email, WhatsApp (+92 307 2024974), phone (tappable tel:), LinkedIn, GitHub. Resume download (served from backend). Social links in footer too.
8. **Footer** — quiet stack credits. "Built with NestJS + Next.js + pgvector; live Swagger at /api/docs." Resume download optional here too.
