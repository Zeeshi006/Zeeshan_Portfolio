...
### D.6.1 Four candidate options (branches hero/option-1 through hero/option-4)

Four implementations exist as separate git branches for visual comparison. Merge the winner into main and delete the rest.

| Branch | Name | Thematic |
|---|---|---|
| `hero/option-1` | Wireframe Topology (Network Graph) | System architecture / knowledge graph |
| `hero/option-2` | Signal-Propagation Waves (Concentric Rings) | Real-time / active communication / voice agents |
| `hero/option-3` | Rotating Wireframe Solid (Octahedron) | Architectural elegance / minimal discipline |
| `hero/option-4` | Glitch / Scan-line Grid | Hacker terminal / signal noise — riskiest, keep subtle |

**Chosen option:** *(pending visual review — update this line after selection)*

To switch: `git checkout hero/option-N && pnpm dev`
To merge winner: `git checkout main && git merge hero/option-N`
...
**Rendering discipline is the difference between 'generic startup 3D' and 'well-engineered system.'** The geometry itself is intentionally minimal (wireframe, simple shapes, low tri-count). The premium feel comes from *how it's rendered*: proper lighting, depth fog, selective bloom, and careful background contrast. This approach respects the visitor's attention (no motion jitter or flashing) while signaling technical mastery.

### §D.6a — Rendering Excellence (Non-negotiable)

The 3D hero must feel premium and depth-rich without being visually complex. This is achieved through *rendering discipline*, not geometry complexity.

**Required rendering properties (implement regardless of which 3D concept is chosen):**

1. **Background Grid Opacity:** `opacity: 0.15` using `--line: #232A36`. Grid lines are a whisper — squint test: grid should nearly disappear, text and 3D shapes should pop.

2. **Depth Fog:** `<fog attach="fog" args={['#0A0C10', 5, 15]} />`. Dampens geometry as it recedes from camera; fog color matches `--ink-900` so it blends seamlessly.

3. **Selective Bloom on Signal-Lime:** `EffectComposer` + `Bloom` from `@react-three/postprocessing`. Intensity `0.3–0.5`, threshold `0.8`. Only signal-lime (`#C6FF3A`) elements have high enough luminance to trigger it. Not the whole scene.

4. **Proper Lighting:** one `directionalLight` (`position={[5,5,5]}`, intensity `1`) + one `ambientLight` (intensity `0.4–0.5`). No point/spot/emissive overload. 3D shapes should read as solid forms with clear shadow and highlight.

5. **Camera:** static, framing geometry slightly off-center in the hero, slight FOV for depth. Not orbiting or moving.

**What NOT to do:**
- ❌ Cursor-tracking particles or mouse-responsive geometry — distracts from text, violates "restrained" principle.
- ❌ Multi-layered depth-panel systems or custom shaders — keep geometry under 500 triangles total.
- ❌ Bloom on the entire scene — bloom must target only signal-lime.
- ❌ Animated waveforms or flowing particles beyond the original 4 concepts.

**Hard performance rules (non-negotiable, all options):**
- Lazy/dynamic import; render hero text + CTAs **immediately**, 3D canvas mounts after first paint.
- Pause the render loop when hero is off-screen (IntersectionObserver).
- Pause when `document.hidden` (tab loses focus).
- Cap device pixel ratio to `[1, 2]`; target 30–60 FPS.
- Geometry: under 500 triangles total. No complex multi-layer depth systems.
- Lighting: one directional + one ambient only.
- Bloom: selective (signal-lime only), intensity 0.3–0.5, threshold 0.8.
- Fog: `<fog args={['#0A0C10', 5, 15]} />` for depth without extra draw calls.
- Static CSS gradient fallback for `prefers-reduced-motion`.
- Lighthouse mobile Performance > 90 is a launch gate. Run after every major rendering change.

Stack: react-three-fiber + drei + @react-three/postprocessing. Keep geometry light; no heavy GLTF models.

### §D.7 — 3D Hero Rendering Checklist (Before Launch)

- [ ] Background grid opacity is `0.15`; nearly invisible at arm's length.
- [ ] Fog enabled: `<fog attach="fog" args={['#0A0C10', 5, 15]} />`. Geometry fades into background.
- [ ] Bloom enabled and selective (lime only). Signal-lime elements glow softly; rest of scene unaffected.
- [ ] Lighting: one directional (position ~[5,5,5], intensity 1) + one ambient (intensity 0.4–0.5).
- [ ] Total 3D geometry: under 500 triangles.
- [ ] No cursor tracking, no animated waveforms beyond original concept, no multi-layer depth systems.
- [ ] `prefers-reduced-motion` → static CSS gradient fallback, no 3D.
- [ ] IntersectionObserver pauses render loop when hero is off-screen.
- [ ] Lighthouse mobile: Performance > 90, Accessibility > 95. ✓
- [ ] Squint test: geometry pops, grid whispers, one lime accent colour, discipline visible.
- [ ] 10-second first impression: reads as backend engineer, not creative studio.

## D.8 Accessibility (launch gate)
...
- **For the 3D hero rendering:** the wow factor comes from *rendering excellence*, not geometry complexity. Implement the required rendering properties in §D.6a: fog, bloom (selective on lime only), proper lighting, background grid opacity 0.15. Do NOT implement cursor-tracking particles, multi-layered depth panels, or complex animated waveforms — these violate the "Terminal Observatory" restraint principle and tank performance. After each rendering change, run Lighthouse mobile; if Performance drops below 90, revert or optimize further.
